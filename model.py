...
class RoomPopulationModel(nn.Module):
    ...
    def __init__(self, input_size=3408, output_size=1608):
        ...
        self.seq = nn.Sequential(
            nn.Linear(input_size, input_size * 4, dtype=torch.float64),
            nn.ReLU(),
            nn.Linear(input_size * 4, input_size, dtype=torch.float64),
            nn.ReLU(),
            nn.Linear(input_size, output_size, dtype=torch.float64),
        )
